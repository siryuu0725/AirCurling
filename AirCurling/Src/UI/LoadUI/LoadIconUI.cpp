...
{
	//座標初期化
	m_loadicon_info.m_ui_pos = m_external_pos[(__int8)LoadIconTexCategory::RoteTex];
...
void LoadIconUI::Update()
{
	...
	//各頂点を回転
	//左上
	m_loadicon_info.left_top_pos.x = Calculation::Rote(rote_pos, center_pos, m_loadicon_info.m_rote_angle).x;
	m_loadicon_info.left_top_pos.y = Calculation::Rote(rote_pos, center_pos, m_loadicon_info.m_rote_angle).z;
	//左下
	m_loadicon_info.left_down_pos.x = Calculation::Rote(rote_pos2, center_pos, m_loadicon_info.m_rote_angle).x;
	m_loadicon_info.left_down_pos.y = Calculation::Rote(rote_pos2, center_pos, m_loadicon_info.m_rote_angle).z;
	//右上																   
	m_loadicon_info.right_top_pos.x = Calculation::Rote(rote_pos3, center_pos, m_loadicon_info.m_rote_angle).x;
	m_loadicon_info.right_top_pos.y = Calculation::Rote(rote_pos3, center_pos, m_loadicon_info.m_rote_angle).z;
	//右下
	m_loadicon_info.right_down_pos.x = Calculation::Rote(rote_pos4, center_pos, m_loadicon_info.m_rote_angle).x;
	m_loadicon_info.right_down_pos.y = Calculation::Rote(rote_pos4, center_pos, m_loadicon_info.m_rote_angle).z;
}

//描画情報送信関数
void LoadIconUI::Draw()
{
	Graphics::Instance()->DrawRoteTexture(&m_loadicon_info.m_ui_tex,
